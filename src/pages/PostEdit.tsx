...
export function PostEdit() {
  ...
  const updatePostStatus = async (status: PostStatus) => {
    ...
    try {
      const apiUrl = `${backendUrl}/api/v1/posts/${post.id}/status`;
      ...
      // Show success message for regeneration
      if (status === 'REGENRATE') {
        alert('✅ Regeneration webhook triggered! The post will be regenerated by n8n workflow. Please check back in a few minutes.');
      }
  ...
  // Trigger regeneration webhook
  const handleRegenerate = async () => {
    if (!post) return;

    // Confirm before regenerating
    const confirmed = window.confirm(
      'Are you sure you want to regenerate this post? This will trigger the n8n workflow to recreate the blog post content.'
    );
    
    if (!confirmed) return;

    try {
      // Update status to REGENRATE which automatically triggers the webhook via backend
